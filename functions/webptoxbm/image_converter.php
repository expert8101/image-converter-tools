...
class Image_converter{
	...
	function convert_image($convert_type, $target_dir, $image_name, $image_quality=100){
		...
		if($convert_type == 'xbm'){
			...
			imagexbm($binary, $target_dir.$img_name.'.'.$convert_type, $image_quality);
			unlink($image);
	...
	public function upload_image($files, $target_dir, $input_name){
		...
		// Check file size - restrict if greater than 50 MB 
		$file_size = $this->check_file_size($files[$input_name]["size"], 20000000);
		if(!$file_size){
		...
		if($imageFileType != "webp") {
			echo '<script>alert("Please upload WEBP Image only!")</script>';
			return false;
		  }
